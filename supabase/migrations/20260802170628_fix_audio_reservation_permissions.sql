-- The Edge Function authenticates the bearer token with admin.auth.getUser()
-- before invoking this service-role-only RPC. Reading auth.users again here
-- made the SECURITY INVOKER function fail because service_role intentionally
-- has no SELECT privilege on Supabase Auth's internal tables.
