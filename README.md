...
- настоящий помощник ответов Google Gemini через защищённую Supabase Edge Function, с проверкой adab до и после генерации;
...
Помощник ответов не подставляет локальные шаблоны: он отправляет только текущее сообщение в Google Gemini через авторизованную Supabase Edge Function. Вход и результат повторно проверяются на запрещённое содержание, давление, непроверенные религиозные утверждения и неподходящую романтическую формулировку. При недоступности модели приложение показывает честную ошибку. Фильтр не является религиозным заключением; итоговый текст нужно перечитать перед отправкой.
