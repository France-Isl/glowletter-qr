...
Без облачного endpoint редактор использует встроенные проверенные сценарии: учитывает смысл сообщения, отношение, стиль, длину и главную мысль пользователя. Cloudflare Workers AI может дать генеративный ответ, после чего сервер повторно применяет фильтр адаба и отклоняет небезопасный результат. Фильтр не является религиозным заключением; итоговый текст нужно перечитать перед отправкой.
...
- `supabase` — схема прогресса, RLS и удаление аккаунта;
- `.github/workflows/mobile-build.yml` — preview-сборка APK и unsigned AAB.

Подробные инструкции: [`mobile/README.md`](mobile/README.md), [`mobile/ios/README.md`](mobile/ios/README.md) и [`backend/cloudflare/README.md`](backend/cloudflare/README.md).
