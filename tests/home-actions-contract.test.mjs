...
// The two primary creation/organizing actions share one row when space allows.
...
assert.match(styles, /\.home-smart-actions\s*\{[^}]*display\s*:\s*grid[^}]*grid-template-columns\s*:\s*minmax\(0,\s*1\.35fr\)\s+minmax\(0,\s*1fr\)[^}]*width\s*:\s*100%/iu);
