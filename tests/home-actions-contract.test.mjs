...
// Letter creation is gone: the home row now carries the Moments entry alone.
assert.doesNotMatch(index, /id=["']aiOpenHome["']/u, "the letter composer entry must stay removed");
assert.match(index, /<button\b(?=[^>]*\bid=["']momentsOpenHome["'])[^>]*\btype=["']button["'][^>]*>/u);
assert.doesNotMatch(index, /id=["']replyOpenHome["']/u);
assert.match(styles, /\.home-smart-actions\s*\{[^}]*display\s*:\s*grid[^}]*grid-template-columns\s*:\s*1fr/iu);
