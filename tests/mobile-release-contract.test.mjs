...
assert.match(android, /versionCode\s*=\s*11\b/);
assert.match(android, /versionName\s*=\s*["']2\.2\.5["']/);
assert.match(android, /exclude\s+["']mobile\/\*\*["'][\s\S]{0,180}["']supabase\/\*\*["'][\s\S]{0,180}["']tests\/\*\*["']/);
assert.match(android, /exclude\s+["']\*\.md["'][\s\S]{0,100}["']\*\.py["']/);
assert.match(ios, /MARKETING_VERSION:\s*2\.2\.5\b/);
assert.match(ios, /CURRENT_PROJECT_VERSION:\s*11\b/);
...
assert.match(workflow, /GlowLetter-2\.2\.5-debug\.apk/);
assert.match(workflow, /GlowLetter-2\.2\.5-preview-debug\.apk/);
assert.match(workflow, /TAG:\s*v2\.2\.5-preview/);
...
assert.match(androidMain, /onPause\(\)[\s\S]{0,260}webView\.onPause\(\)[\s\S]{0,500}webView\.pauseTimers\(\)[\s\S]{0,160}super\.onPause\(\)/);
...
console.log(JSON.stringify({ ok: true, android: "2.2.5 (11)", ios: "2.2.5 (11)", previewTag: "v2.2.5-preview", nativeShare: true, autoFullscreen: true }));
